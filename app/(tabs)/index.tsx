...
import { View, StyleSheet, TouchableOpacity, Text, TextInput, FlatList, ActivityIndicator, Keyboard, Animated, Easing, Platform, Dimensions, ScrollView } from 'react-native';
import MapView, { Marker, PROVIDER_GOOGLE, Polyline } from 'react-native-maps';
import * as Location from 'expo-location';
import { Magnetometer } from 'expo-sensors';
import { Colors } from '@/constants/Colors';
import { Search, LocateFixed, Plane, X, ArrowRight, Save, MapPin, RefreshCcw, Layers, Send, Navigation, CornerUpLeft, CornerUpRight, ArrowUp, Clock, Bookmark, Check } from 'lucide-react-native';
...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (magnetometerSubscription.current) magnetometerSubscription.current.remove();
    };
  }, []);
  ...
  const handleSaveFlight = (item: any) => {
    ...
    set(newRef, {
      ...
      setAlertConfig({ visible: true, type: 'error', title: 'Gagal', message: 'Gagal menyimpan.' });
    });
  };
  ...
  const handleCenterUser = () => {
    const targetLoc = location || locationRef.current;
    if (targetLoc && mapRef.current) {
      if (isDriveMode) {
        mapRef.current.animateCamera({ center: { latitude: targetLoc.coords.latitude, longitude: targetLoc.coords.longitude }, pitch: 65, heading: 0, zoom: 19 });
      } else {
        mapRef.current.animateToRegion({ latitude: targetLoc.coords.latitude, longitude: targetLoc.coords.longitude, latitudeDelta: 0.01, longitudeDelta: 0.01 }, 1000);
      }
    }
  };
  ...
  return (
    <View style={styles.container}>
              ...
              <View style={styles.popupCard}>
                ...
                <View style={styles.actionRow}>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: Colors.light.primary, flex: 1 }]} onPress={() => {
                    startDriveMode(selectedAirport);
                  }}>
                    <Navigation size={18} color="white" strokeWidth={2.5} /><Text style={styles.actionText}>Mulai Navigasi</Text>
                  </TouchableOpacity>
                </View>

                <View style={styles.flightListSection}>
                  <Text style={styles.flightListTitle}>Keberangkatan Live</Text>
                  {isFlightLoading ? (
                    ...
                    <View style={{ maxHeight: 150 }}>
                      <ScrollView showsVerticalScrollIndicator={true} nestedScrollEnabled={true}>
                        {flightSchedule.map((flight, idx) => (
                          <View key={idx} style={styles.miniFlightCard}>
                            <View style={{ flex: 1 }}>
                              <Text style={styles.miniFlightCode}>{flight.airline} ({flight.flightNumber})</Text>
                              <Text style={styles.miniFlightDest}>Ke: {flight.destination.city}</Text>
                              <Text style={styles.miniFlightTime}>{flight.time} • {flight.status}</Text>
                            </View>
                            <TouchableOpacity onPress={() => handleSaveFlight(flight)} style={styles.miniSaveBtn}>
                    ...
                    <Text style={styles.noFlightText}>Tidak ada data penerbangan saat ini.</Text>
                  )}
                </View>
      ...
      <MapView
        ...
        onRegionChangeComplete={(region) => { if (!isDriveMode) setCurrentRegion(region); }}
...
const styles = StyleSheet.create({
  ...
  loadingBox: { backgroundColor: 'transparent', padding: 24, borderRadius: 16, alignItems: 'center' },
  loadingText: { color: 'white', marginTop: 16, fontFamily: 'Poppins_600SemiBold', fontSize: 16 },
  ...
  flightListSection: { marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#F3F4F6' },
  flightListTitle: { fontFamily: 'Poppins_600SemiBold', fontSize: 13, color: Colors.light.text, marginBottom: 8 },
  miniFlightCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F9FAFB', padding: 10, borderRadius: 12, marginBottom: 8 },
  miniFlightCode: { fontFamily: 'Poppins_700Bold', fontSize: 12, color: Colors.light.text },
  miniFlightDest: { fontFamily: 'Poppins_400Regular', fontSize: 11, color: Colors.light.icon },
  miniFlightTime: { fontFamily: 'Poppins_500Medium', fontSize: 10, color: '#10B981', marginTop: 2 },
  miniSaveBtn: { padding: 8 },
  noFlightText: { fontFamily: 'Poppins_400Regular', fontSize: 11, color: '#9CA3AF', fontStyle: 'italic' },
